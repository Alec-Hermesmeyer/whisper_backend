...
exports.Eagle = exports.EagleProfiler = void 0;
const fs = require("fs");
...
class EagleProfiler {
    ...
    constructor(accessKey, options = {}) {
        ...
        if (accessKey === null ||
            accessKey === undefined ||
            accessKey.length === 0) {
            throw new errors_1.EagleInvalidArgumentError(`No AccessKey provided to Eagle Profiler`);
        }
        const { modelPath = path.resolve(__dirname, DEFAULT_MODEL_PATH), libraryPath = (0, platforms_1.getSystemLibraryPath)(), } = options;
        if (!fs.existsSync(libraryPath)) {
            throw new errors_1.EagleInvalidArgumentError(`File not found at 'libraryPath': ${libraryPath}`);
        }
        if (!fs.existsSync(modelPath)) {
            throw new errors_1.EagleInvalidArgumentError(`File not found at 'modelPath': ${modelPath}`);
        }
        const pvEagle = require(libraryPath); // eslint-disable-line
        this._pvEagle = pvEagle;
        let eagleProfilerAndStatus = null;
        try {
    ...
    enroll(pcm) {
        ...
        if (this._profiler === 0 ||
            ...
            throw new errors_1.EagleInvalidStateError('Eagle Profiler is not initialized');
        }
        if (pcm === undefined || pcm === null) {
            throw new errors_1.EagleInvalidArgumentError(`PCM array provided to 'Eagle.enroll()' is undefined or null`);
        }
        else if (pcm.length < this._minEnrollSamples) {
            throw new errors_1.EagleInvalidArgumentError(`PCM size (${pcm.length}) must be greater than 'Eagle.minEnrollSamples' (${this.minEnrollSamples})`);
        }
    ...
    reset() {
        ...
        if (status && status !== pv_status_t_1.default.SUCCESS) {
    ...
    handlePvStatus(status, message) {
        ...
        if (errorObject.status === pv_status_t_1.default.SUCCESS) {
            ...
            (0, errors_1.pvStatusToException)(status, 'Unable to get Eagle Profiler error state');
        }
    }
}
exports.EagleProfiler = EagleProfiler;
class Eagle {
    _pvEagle;
    _handle;
    _version;
     ...
     * @param {string} options.libraryPath the path to the Eagle library (.node extension)
     */
    constructor(accessKey, speakerProfiles, options = {}) {
        assert(typeof accessKey === 'string');
        if (accessKey === null ||
            accessKey === undefined ||
            accessKey.length === 0) {
            throw new errors_1.EagleInvalidArgumentError(`No AccessKey provided to Eagle`);
        }
        const { modelPath = path.resolve(__dirname, DEFAULT_MODEL_PATH), libraryPath = (0, platforms_1.getSystemLibraryPath)(), } = options;
        if (!fs.existsSync(libraryPath)) {
            throw new errors_1.EagleInvalidArgumentError(`File not found at 'libraryPath': ${libraryPath}`);
        ...
        const pvEagle = require(libraryPath); // eslint-disable-line
        this._pvEagle = pvEagle;
        let eagleHandleAndStatus = null;
        const numSpeakers = !Array.isArray(speakerProfiles) ? 1 : speakerProfiles.length;
        try {
            pvEagle.set_sdk('nodejs');
            eagleHandleAndStatus = pvEagle.init(accessKey, modelPath, numSpeakers, !Array.isArray(speakerProfiles) ? [speakerProfiles] : speakerProfiles);
        }
        catch (err) {
            (0, errors_1.pvStatusToException)(pv_status_t_1.default[err.code], err);
        }
    ...
    process(pcm) {
        ...
        if (this._handle === 0 ||
            ...
            throw new errors_1.EagleInvalidStateError('Eagle is not initialized');
        }
        if (pcm === undefined || pcm === null) {
            throw new errors_1.EagleInvalidArgumentError(`PCM array provided to 'Eagle.process()' is undefined or null`);
        }
        else if (pcm.length !== this.frameLength) {
            throw new errors_1.EagleInvalidArgumentError(`Size of frame array provided to 'Eagle.process()' (${pcm.length}) does not match the engine 'Eagle.frameLength' (${this.frameLength})`);
        }
    ...
    handlePvStatus(status, message) {
        ...
        if (errorObject.status === pv_status_t_1.default.SUCCESS) {
            ...
            (0, errors_1.pvStatusToException)(status, 'Unable to get Eagle error state');
        }
    }
}
exports.Eagle = Eagle;
